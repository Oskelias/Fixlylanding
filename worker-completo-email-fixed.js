...
/**
 * 🆕 FUNCIÓN DE EMAIL MEJORADA - v4.2.0
 * Incluye retry logic, mejor logging y manejo de errores específicos
 */
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (response.ok) {
        ...
        // Si es 401, no reintentar (problema de configuración SPF)
...
async function handleGenerateCode(request, env) {
  try {
    ...
    // 2. Email al usuario (credenciales) - 🆕 MEJORADO
...
function handleHealth(request) {
  return new Response(JSON.stringify({
    ...
    version: '4.2.0-email-fixed',  // 🆕 NUEVA VERSIÓN
