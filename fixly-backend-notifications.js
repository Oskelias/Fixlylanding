/**
 * FIXLY BACKEND - VERSION 4.0.0 NOTIFICATIONS
 * Cloudflare Worker con Telegram Bot + MailChannels
 * 
 * NUEVAS FUNCIONALIDADES:
 * - ✅ Notificaciones Telegram automáticas
 * - ✅ Emails automáticos con MailChannels  
...
// ==========================================
// ENDPOINT: GENERATE CODE CON NOTIFICACIONES
// ==========================================
async function handleGenerateCode(request, env) {
  try {
    const data = await request.json();
    const { empresa, email, telefono } = data;
    ...
    const fechaExpiracion = new Date(Date.now() + 15 * 24 * 60 * 60 * 1000); // 15 días
    ...
    return new Response(JSON.stringify({
      ...
      tenantId,
      fechaExpiracion,
      mensaje: 'Código generado exitosamente. Revisa tu email.',
      credenciales: {
        usuario: username,
        contraseña: password
      },
...
async function handleLogin(request, env) {
  try {
    ...
    if (!userData) {
      ...
      }), { status: 401, headers: { 'Content-Type': 'application/json', ...handleCORS(request) } });
    }

    // Verificar contraseña (en este caso, está en la respuesta del generate-code)
    // Para mayor seguridad, deberías hashear las contraseñas
...
function handleHealth(request) {
  return new Response(JSON.stringify({
    ...
    features: ['telegram', 'email', 'multi-tenant', 'mercadopago'],
    endpoints: [
